  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        ...
        <div className="overflow-x-auto overscroll-x-contain" tabIndex={0} aria-label="Withdrawals table, horizontally scrollable">
          <table className="w-full min-w-[920px] table-fixed text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? <tr><td colSpan={6} className="px-5 py-12 text-center text-slate-500" role="status">Loading withdrawals…</td></tr> : items.length === 0 ? <tr><td colSpan={6} className="px-5 py-12 text-center text-slate-500">No withdrawals match these filters.</td></tr> : items.map((row) => <tr key={row.id} className="hover:bg-slate-50/70"><td className="px-5 py-4"><CopyableCell value={ownerName(row)} label="owner name" className="font-medium text-slate-900"/><p className="text-xs text-slate-500">{ownerType(row)}</p></td><td className="px-5 py-4"><CopyableCell value={`${row.bankAccount.bankName} · ${maskAccount(row.bankAccount.accountNumber)} · ${row.bankAccount.accountName}`} label="withdrawal destination" lines={2}/></td><td className="px-5 py-4 font-semibold text-slate-950"><CopyableCell value={formatNaira(row.amount)} label="withdrawal amount"/></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles[row.status] || "bg-slate-100"}`}>{row.status.replaceAll("_", " ")}</span></td><td className="px-5 py-4 text-slate-600"><CopyableCell value={`${formatDate(row.createdAt)} ${formatTime(row.createdAt)}`} label="request date"/></td><td className="px-5 py-4 text-right"><button onClick={() => setSelected(row)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500"><Eye className="h-3.5 w-3.5"/>Details</button></td></tr>)}
            </tbody>
          </table>
        </div>
