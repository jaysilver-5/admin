...
import { ListLoadingState } from "@/components/dashboard/ui/LoadingState";
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <section className="rounded-2xl border border-[#EAECF0] bg-white p-6">
        ...
        <div className="overflow-x-auto overscroll-x-contain" tabIndex={0} aria-label="Notification campaigns table, horizontally scrollable"><table className="w-full min-w-[900px] table-fixed text-sm"><colgroup><col className="w-[26%]"/><col className="w-[15%]"/><col className="w-[13%]"/><col className="w-[11%]"/><col className="w-[13%]"/><col className="w-[16%]"/><col className="w-[6%]"/></colgroup><thead><tr className="border-b border-[#EAECF0] text-left text-[#667085]"><th className="px-3 py-3">Campaign</th><th className="px-3 py-3">Audience</th><th className="px-3 py-3">Channels</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Results</th><th className="px-3 py-3">Date</th><th className="px-3 py-3"></th></tr></thead><tbody>{campaigns.map((campaign) => <tr key={campaign.id} className="border-b border-[#F2F4F7]"><td className="px-3 py-4"><CopyableCell value={campaign.title} label="campaign title" className="font-medium text-[#101828]"/><CopyableCell value={campaign.body} label="campaign message" className="mt-1 text-xs text-[#667085]"/></td><td className="px-3 py-4"><CopyableCell value={audienceLabel(campaign)} label="audience"/></td><td className="px-3 py-4"><CopyableCell value={campaign.channels.join(", ")} label="channels"/></td><td className="px-3 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(campaign.status)}`}>{campaign.status}</span></td><td className="px-3 py-4"><span className="text-emerald-700">{campaign.sentCount} sent</span>{campaign.failedCount > 0 && <span className="ml-2 text-red-600">{campaign.failedCount} failed</span>}</td><td className="px-3 py-4"><CopyableCell value={`${formatDate(campaign.scheduledAt || campaign.createdAt)} ${formatTime(campaign.scheduledAt || campaign.createdAt)}`} label="campaign date"/></td><td className="px-3 py-4 text-right">{campaign.status === "SCHEDULED" && <button onClick={() => cancelCampaign(campaign.id)} className="text-sm font-medium text-red-600 focus:outline-none focus:ring-2 focus:ring-blue-500">Cancel</button>}</td></tr>)}{campaigns.length === 0 && <tr><td colSpan={7} className="py-10 text-center text-[#667085]">No campaigns yet.</td></tr>}</tbody></table></div>
      </section>
    </div>
  );
}
