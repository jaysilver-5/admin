...
export default function SupportTab() {
  ...
  return <div className="space-y-4 bg-white p-4">
    ...
    <div className="flex items-center gap-3"><div className="relative w-full max-w-[420px]"><input value={query} onChange={(e) => setQuery(e.target.value)} aria-label="Search support conversations" placeholder="Search by name, time" className="h-[42px] w-full rounded-md border border-gray-200 bg-white pl-3 pr-10 text-sm placeholder:text-gray-400 focus:ring-1 focus:ring-blue-500" /><button type="button" className="absolute right-1 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-md hover:bg-gray-50" aria-label="Filter conversations"><Filter className="h-4 w-4 text-gray-500" /></button></div></div>
