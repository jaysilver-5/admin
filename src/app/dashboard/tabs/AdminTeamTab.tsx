...
export default function AdminTeamTab() {
  ...
  const toggleStatus = async (member: AdminTeamMember) => {
    ...
    try {
      await updateAdminStatus(member.id, next, next ? undefined : "Disabled from the administrator team panel");
      load();
  ...
  return (
    <div className="space-y-6">
      ...
      {message && <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800" role="status">{message}</div>}
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {roles.map((role) => <div key={role.id} className="rounded-2xl border border-slate-200 bg-white p-4"><Shield className="h-5 w-5 text-blue-700"/><h2 className="mt-3 font-semibold text-slate-900">{role.name}</h2><p className="mt-1 min-h-10 text-xs text-slate-500">{role.description}</p><p className="mt-3 text-xs font-medium text-slate-600">{role._count.assignments} assigned · {role.permissions.length} capabilities</p></div>)}
      </div>
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="border-b border-slate-100 px-5 py-4"><h2 className="font-semibold text-slate-900">Team members</h2></div>
        <div className="overflow-x-auto overscroll-x-contain" tabIndex={0} aria-label="Administrator team table, horizontally scrollable">
          <table className="w-full min-w-[780px] table-fixed text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && members.length === 0 && <tr><td colSpan={5} className="px-5 py-12 text-center text-slate-500">No administrators found.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      <ModalBase open={Boolean(selected)} onClose={() => setSelected(null)} width={512} ariaLabel="Edit administrator roles">
        {selected && <div className="p-6"><div className="flex items-center gap-3"><div className="rounded-xl bg-blue-50 p-2"><UserCog className="h-5 w-5 text-blue-700"/></div><div className="min-w-0"><h2 className="font-semibold text-slate-950">Edit administrator roles</h2><p className="truncate text-xs text-slate-500">{selected.account.email || selected.account.phone}</p></div></div><div className="mt-5 space-y-2">{roles.map((role) => <label key={role.key} className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-3 hover:bg-slate-50"><input type="checkbox" checked={roleKeys.includes(role.key)} onChange={(e) => setRoleKeys((keys) => e.target.checked ? [...keys, role.key] : keys.filter((key) => key !== role.key))} className="mt-1"/><span><span className="block text-sm font-semibold text-slate-900">{role.name}</span><span className="text-xs text-slate-500">{role.description}</span></span></label>)}</div><div className="mt-6 flex justify-end gap-2"><button onClick={() => setSelected(null)} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button disabled={!roleKeys.length} onClick={() => void save()} className="rounded-xl bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">Save roles</button></div></div>}
      </ModalBase>
    </div>
  );
}
